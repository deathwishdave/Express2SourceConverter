...
ExpressHighlighter::ExpressHighlighter(QTextDocument *parent) : QSyntaxHighlighter(parent)
 {
	HighlightingRule rule;

	{
		m_keywordFormat.setForeground( QColor( 127, 0, 85 ) );
		//m_keywordFormat.setFontWeight(QFont::Bold);
		QStringList keywordPatterns;
		keywordPatterns 
			<< "\\b(ABSTRACT )?SUPERTYPE OF\\b" 
			<< "\\bSUBTYPE OF\\b"
			<< "\\bWHERE\\b"
			<< "\\bDERIVE\\b"
			<< "\\bINVERSE\\b"
...
void ExpressHighlighter::highlightBlock(const QString &text)
 {
